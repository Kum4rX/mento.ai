...
export default function Avatar3D({ size = 'md', isActive = false, className = '' }: Avatar3DProps) {
  ...
  return (
    ...
    >
      {/* Glow effect */}
      <div className="absolute inset-0 rounded-full bg-gradient-to-r from-mentor-primary to-mentor-secondary opacity-20 blur-xl animate-pulse" />
      ...
      {isActive && (
        <div className="absolute inset-0 rounded-full">
          <div className="absolute inset-0 rounded-full border-2 border-mentor-primary animate-ping opacity-75" />
          <div className="absolute inset-4 rounded-full border border-mentor-secondary animate-ping opacity-50" style={{ animationDelay: '0.5s' }} />
        </div>
      )}
    </motion.div>
  );
