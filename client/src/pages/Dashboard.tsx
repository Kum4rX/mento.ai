    ...
    <div className="min-h-screen bg-gradient-to-br from-mentor-surface via-background to-mentor-surface p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        >
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-muted-foreground text-sm sm:text-base">
              Ready to learn? Choose a subject to launch your 1-on-1 AI tutor with real-time doubt clarification.
            </p>
          </div>
        ...
        >
          ...
          <Card className="glass border-border/40 rounded-2xl shadow-sm">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">{stats.totalSessions || 0} lifetime total</p>
              </div>
          ...
          <Card className="glass border-border/40 rounded-2xl shadow-sm">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-extrabold text-foreground">{stats.totalDurationMinutes || 0} <span className="text-base font-normal text-muted-foreground">min</span></p>
          ...
          {/* Card 3: Unique Topics Explored */}
          <Card className="glass border-border/40 rounded-2xl shadow-sm">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Topics Explored</p>
                <p className="text-3xl font-extrabold text-foreground">{stats.uniqueTopics || (stats.totalSessions ? 1 : 0)}</p>
                <p className="text-xs text-muted-foreground">across academic subjects</p>
              </div>
          ...
          <Card className="glass border-border/40 rounded-2xl shadow-sm">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">AI Doubt Clarity</p>
                <p className="text-3xl font-extrabold text-foreground">100%</p>
                <p className="text-xs text-green-500 font-medium">1-on-1 Step-by-Step</p>
              </div>
            ...
            <Card className="glass border-border/40 rounded-3xl shadow-sm overflow-hidden">
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Instant contextual AI video sessions with your subject tutor</CardDescription>
                  </div>
                  <Button 
                    variant="ghost" 
                    size="sm"
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {quickLaunchSubjects.map((item) => (
                    <div
                      key={item.title}
                      onClick={() => navigate(`/session?subject=${encodeURIComponent(item.title)}&topic=${encodeURIComponent(item.topic)}`)}
                ...
                {recentSessions.length > 0 ? (
                  <div className="space-y-2.5">
                    {recentSessions.map((rec) => (
                      <div
                        key={rec._id}
                        onClick={() => navigate(`/session?subject=${encodeURIComponent(rec.subject)}&topic=${encodeURIComponent(rec.topic)}`)}
